...
# ---------- Combo generator (size 1~3 auto by budget) ----------
def recommend_combos(df, chosen_tags, sweet, budget, topk=3):
    cand = ranked_items(df, chosen_tags, sweet).head(12)  # limit for speed
    combos = []
    ...
    combos.sort(key=lambda x: (-x[2], x[1], -x[3]))  # high score, cheaper, more items slight preference
    # deduplicate by names signature
    out = []
    seen = set()
